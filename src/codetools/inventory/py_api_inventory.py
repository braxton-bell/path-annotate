...
from codetools.annotate.path_annotate import ConsoleManager, PathHeaderAnnotator
...
class PythonApiSignatureExtractor:
    ...
    def run(self) -> "InventoryReport":
        ...
        pkg_name, pkg_version = self._read_pyproject(self.config.get("_pyproject_path"))
        ...
        for pkg in packages:
            ...
            pkg["modules"] = sorted(processed_modules, key=lambda m: m["qname"])
    ...
    def _build_report(
        ...
        return InventoryReport(
            meta=meta, stats=stats, packages=sorted(packages, key=lambda p: p["qname"])
        )
    ...
    def _build_package_tree(self, files: List[Path]) -> List[PackageRecord]:
        ...
        if package_mode == "require_init_py":
            package_dirs = {p.parent for p in files if p.name == "__init__.py"}
        ...
        for pkg_path in sorted(list(package_dirs)):
            ...
            packages_map[rel_path] = PackageRecord(
                path=rel_path, qname=qname, is_package=True, modules=[]
            )
    ...
    def _read_pyproject(
        self, pyproject_path_str: Optional[str]
        ...
        if not pyproject_path.is_file():
            self.logger.warn(f"Pyproject file not found at: {pyproject_path}")
            return None, None
    ...
    def write_yaml(
        ...
        if to_stdout:
            ...
            try:
                ...
                self.logger.info(f"Successfully wrote {out_p}")
            except Exception as e:
    ...
    def _parse_module_content(
        ...
        try:
            ...
            return (str(file_path), f"Error calculating relative path: {e}")
    ...
    def _extract_nodes(
        ...
        List[ConstantRecord], List[EnumRecord], List[FunctionRecord], List[ClassRecord]
    ]:
        ...
        for node in tree_body:
            ...
            if (
                config.get("include_constants")
                and isinstance(node, (ast.Assign, ast.AnnAssign))
                and (target := getattr(node, "target", None))
            ...
            # 3. Classes (and Enums)
                ...
                # Check for Enums
                if config.get("include_enums") and PythonApiSignatureExtractor._is_enum(
                    node
                ):
    ...
    def _build_constant_record(
        ...
        if strategy == "uppercase":
            if not (
                name.isupper() and re.match(r"^[A-Z0-9_]+$", name) and vis == "public"
            ):
                return None
    ...
    def _build_enum_record(
        node: ast.ClassDef, qname: str, config: Dict[str, Any]
    ) -> EnumRecord:
        ...
        for item in node.body:
            ...
            if isinstance(item, (ast.Assign, ast.AnnAssign)):
                ...
                if isinstance(target, ast.Name):
                    ...
                    _val, value_repr = (
                        PythonApiSignatureExtractor._extract_literal_value(
                            getattr(item, "value", None)
                        )
                    )
                    members.append(EnumMemberRecord(name=name, value_repr=value_repr))
    ...
    def _build_method_record(
        ...
        sig = PythonApiSignatureExtractor._build_signature(node.args, node.returns)
    ...
    def _build_signature(
        args: ast.arguments, returns: Optional[ast.expr]
    ) -> FunctionSignature:
        ...
        return FunctionSignature(
            params=params, returns=PythonApiSignatureExtractor._unparse_node(returns)
        )

    @staticmethod
    def _extract_literal_value(node: Optional[ast.expr]) -> Tuple[Optional[Any], str]:
        """
...
def main():
    ...
    # --- Core ---
    parser.add_argument(
        "--root", required=True, help="Top-level directory to traverse."
    )
    parser.add_argument("--config", help="Path to a JSON/JSONC configuration file.")
    parser.add_argument(
        "-e",
        "--exclude",
        action="append",
    ...
    parser.add_argument(
        "--no-color", action="store_true", help="Disable colorized console output."
    )
    parser.add_argument(
        "--print-summary",
        action="store_true",
